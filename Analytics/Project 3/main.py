...
#========================================================================================================================
# 👉 Part 5.1: Feature Creation
# .1 Create Height Category column: Height >= 1000 → "Very High", 800-999 → "High", < 800 → "Moderate".
condition=[df['Height'] > 999 ,(df['Height'] <= 999) & (df['Height'] >= 800), df['Height'] < 800]
...
# 👉 Part 5.2: NumPy Usage
# .2 Convert Height column to NumPy array.
height_array=df['Height'].to_numpy()

# .3 Calculate height differences using np.diff().
height_growth=np.diff(height_array)#here we are checking the difference between the heights of the hills
print(height_growth)
...
# 5.4 Save graphs: plt.savefig("height_trend.png"), plt.savefig("height_category_stacked.png"), plt.savefig("height_histogram.png")

# 5.5 Identify: Which region has the tallest hills, which category is most common, and the distribution pattern of heights.
